...
// Archivos de datos - usar process.cwd() para el ejecutable
const DHL_DATA_FILE = path.join(process.cwd(), 'fuel-data.json');
const GLS_DATA_FILE = path.join(process.cwd(), 'gls-fuel-data.json');
...
async function fetchGLSFuelSurcharge() {
    try {
        ...
        const response = await fetch('https://www.clickgasoil.com/c/precio-gasoleo-a-catalua', {
            headers: {
        ...
        if (fs.existsSync(GLS_DATA_FILE)) {
            try {
                ...
                console.log(`📦 Usando valor GLS guardado: ${saved.rate}%`);
                return saved.rate;
            } catch (readError) {
...
async function fetchDHLSurcharge() {
    try {
        ...
        const response = await fetch('https://www.dhl.com/es-es/ecommerce/home/business-to-business-shipments/custom-solutions/fuel-surcharge.html', {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        ...
        // Buscar cualquier porcentaje como fallback
        if (foundRates.length === 0) {
            ...
            if (allPercentages) {
                ...
                if (percentages.length > 0) {
                    ...
                    foundRates.push({ month: 'detectado', rate: highest, fullDate: `valor ${highest}%` });
        ...
        let selectedRate = foundRates.find(item => item.month === target.month);
        
        if (!selectedRate && foundRates.length > 0) {
            selectedRate = foundRates[foundRates.length - 1];
            console.log(`⚠️ Usando el más reciente: ${selectedRate.rate}%`);
        }
        ...
        console.log(`✅ Combustible DHL actualizado: ${selectedRate.rate}%`);
        ...
        if (fs.existsSync(DHL_DATA_FILE)) {
            try {
                ...
                console.log(`📦 Usando valor DHL guardado: ${saved.rate}%`);
                return saved.rate;
            } catch (readError) {
                console.error('Error al leer archivo guardado:', readError.message);
            }
...
app.get('/api/dhl-fuel', (req, res) => {
    try {
        if (fs.existsSync(DHL_DATA_FILE)) {
            ...
            res.json({ rate: DEFAULT_DHL_RATE, status: 'pending' });
        }
    } catch (error) {
...
app.get('/api/gls-fuel', (req, res) => {
    try {
        if (fs.existsSync(GLS_DATA_FILE)) {
            ...
            res.json({ rate: DEFAULT_GLS_RATE, status: 'pending' });
        }
    } catch (error) {
